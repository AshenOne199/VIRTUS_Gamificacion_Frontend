...
export default function ClaseIndividualProfesor() {
  ...
  const handleAddQuestionUnica = () => {
    setLocalPreguntas((prevPreguntas) => [
      ...prevPreguntas,
      nuevaPreguntaUnica,
    ]);
    setNuevaPreguntaUnica({
  ...
  const handleAnswerChangeMultiple = (index) => (event) => {
    ...
    if (event.target.checked) {
      ...
      newAnswers = nuevaPreguntaMultiple.respuesta.filter(
        (answerIndex) => answerIndex !== index
      );
    }
    setNuevaPreguntaMultiple({
      ...nuevaPreguntaMultiple,
      respuesta: newAnswers,
    });
  };
  ...
  const [crearActivity, setCrearActivity] = useState({
    //Form
    nameForm: "",
    recompensaForm: "",
    castigoForm: "",
  ...
  const postCrucigrama = async (e) => {
    ...
    for (let i = 0; i < filas; i++)
      for (let j = 0; j < columnas; j++) matriz[i][j] = values[`${i},${j}`];
    console.log(matriz);
  ...
  const getActivity = async (filtro) => {
    ...
    let actividadesFiltradas = actividadesBD.filter(
      (actividad) => actividad.claseFK === filtro
    );
  ...
  const eventPostActivity = async () => {
    ...
    try {
      ...
      getActivity(
        JSON.parse(sessionStorage.getItem("ProfesorClaseActual"))._id
      );
    } catch (error) {
  ...
  return (
    <div className="infoClase">
      ...
      <p>
        Nombre clase:{" "}
        {JSON.parse(sessionStorage.getItem("ProfesorClaseActual")).nombre}
      </p>
      <p>
        Codigo del grupo:{" "}
        {JSON.parse(sessionStorage.getItem("ProfesorClaseActual")).codigoGrupo}
      </p>
      <p>
        Descripción:{" "}
        {JSON.parse(sessionStorage.getItem("ProfesorClaseActual")).descripcion}
      </p>
      <p>
        Dificultad:{" "}
        {JSON.parse(sessionStorage.getItem("ProfesorClaseActual")).dificultad}
      </p>
      <hr></hr>
      ...
      <div className="infoActividades">
        ...
        <div className="contenidoInfoActividades">
          ...
          {activity.map((actividad) => {
            return (
              <div key={actividad._id} id={actividad._id}>
                {" "}
                <FichaActividad actividad={actividad}></FichaActividad>{" "}
              </div>
            );
          })}
        </div>
      ...
      <div className="crearClase">
        ...
        <div>
          <div className="formularioActividad">
            <h1 className="login_titleProfesor">
                    ...
                    <input
                      ...
                      onChange={(e) => {
                        const nuevasPalabras = [
                          ...crearActivity.palabrasSopaLetras,
                        ];
              ...
              {crearActivity.tipoJuegoForm === "preguntas" && (
                <>
                  ...
                  >
                    ...
                    <option value="multiple-respuesta">
                      Multiple respuesta
                    </option>
                  ...
                  {crearActivity.tipoPreguntaForm === "unica-respuesta" && (
                    <>
                      ...
                      />
                      <button onClick={handleAddQuestionUnica}>
                        Agregar pregunta
                      </button>
                    ...
                    <>
                      ...
                      {Array.from({ length: 4 }).map((_, index) => (
                        <div key={index}>
                          ...
                          <input
                            type="checkbox"
                            checked={nuevaPreguntaMultiple.respuesta.includes(
                              index
                            )}
                      ...
                      />
                      <button onClick={handleAddQuestionMultiple}>
                        Agregar pregunta
                      </button>
                      {localPreguntas.map((pregunta, index) => (
                        <div key={index}>
                          ...
                          <p>
                            ...
                            {pregunta.respuesta
                              .map(
                                (answerIndex) => pregunta.opciones[answerIndex]
                              )
                              .join(", ")}
                          </p>
                          <p>Tiempo: {pregunta.tiempo}</p>
